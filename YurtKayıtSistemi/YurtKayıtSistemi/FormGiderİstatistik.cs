...
{
    public partial class FormGiderİstatistik : Form
    {
        public FormGiderİstatistik()
        {
            InitializeComponent();
        }

        Sqlbaglantim bgl = new Sqlbaglantim();
        private void FormGiderİstatistik_Load(object sender, EventArgs e)
        {
            ...
            while (oku2.Read())
            {
                comboAylar.Items.Add(oku2[0].ToString());
            }
            bgl.baglanti().Close();
        ...
        private void comboAylar_SelectedIndexChanged(object sender, EventArgs e)
        {
            SqlCommand komut = new SqlCommand("select sum(OdemeMiktar) From Kasa where OdemeAy=@p1", bgl.baglanti());
            komut.Parameters.AddWithValue("@p1", comboAylar.Text);
            SqlDataReader oku = komut.ExecuteReader();
            while (oku.Read())
            {
